extends WeaponState
class_name WeaponCycleDownState

func _on_cycle_down_state_entered() -> void:
	weapon_controller.cycle_weapon(-1)# wheel down = next
	#TODO: await animation to finish playing before allowing onIdle
	weapon_controller.weapon_state_chart.send_event("onIdle")


func _on_cycle_down_state_processing(_delta: float) -> void:
