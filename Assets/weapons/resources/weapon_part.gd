...
func _get_property_list() -> Array[Dictionary]:
	...
	props.append({
		...
		"hint_string": ",".join(Enums.TriggerMode.keys())
	})
	#props.append(_float_prop("reload_speed"))
	
	# ---- Burst-only ----
	if burst_size >= 2:
		props.append(_float_prop("burst_per_shot"))
		props.append(_float_prop("burst_seperation"))

	# ---- Auto / Semi ----
	if trigger_mode in [Enums.TriggerMode.SEMI, Enums.TriggerMode.AUTO]:
		props.append(_float_prop("shot_interval"))

	# ---- Pump only ----
	if trigger_mode == Enums.TriggerMode.PUMP:
		props.append(_float_prop("cycle_time"))

	# ---- Single-load ----
	if trigger_mode == Enums.TriggerMode.SINGLE:
		pass

	return props

func _float_prop(name: String) -> Dictionary:
	return {
		"name": name,
		"type": TYPE_FLOAT
	}
