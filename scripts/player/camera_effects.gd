...
@export var run_pitch : float = 0.15 #Degrees
...
@export var fall_kick_strength = 2.0
...
func calculate_view_offset(delta):
	...
	if speed > 0.1 and player.is_on_floor():
		...
		_step_timer = 0.0
	var bob_sin = sin(_step_timer * 2.0 * PI) * bob_magnitude
	...
	if enable_damage_kick:
		...
		#damage_ratio = ease(damage_ratio, -2) #use this to ease over time
